...
用途：
1) 合并 data/ 目录下多个 worker 生成的 JSONL 分片。
2) 复用 generate_data.py 中的校验逻辑，过滤异常样本。
3) 按最终 Prompt 去重，输出可直接训练的 JSONL。
"""
...
from generate_data import extract_final_prompt, normalize_record, validate_sample
